...
def process_shard(
    ...
    def _flush_batch(f_out):
        ...
        for (instance_key, msg_idx, formatted_gt, gt_keys, duration), (w_s, w_e) in zip(pending_jobs, pending_window_ranges):
            ...
            num_negatives = random.randint(0, max(0, int(duration * max_neg_per_sec)))
            gt_count = len(formatted_gt)
            if gt_count > 0:
                max_negatives = max(0, TERM_MAP_MAX_SIZE - gt_count)
            else:
                max_negatives = TERM_MAP_MAX_SIZE
            selected_negatives = negatives[:min(num_negatives, max_negatives)]
            ...
            if chunk_log_counter < debug_per_chunk_log_limit:
                print(
                    ...
                    f"random_neg_target={num_negatives} selected_negatives={len(selected_negatives)} "
...
def main():
    ...
    parser.add_argument("--max-neg-per-sec", type=int, default=9)
