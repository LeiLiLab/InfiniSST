...
ckpt_dir=/compute/babel-5-23/siqiouya/runs/en-zh/8B-traj-s2-v3.4/last.ckpt/
src_segment_size=$(($SLURM_ARRAY_TASK_ID * 960))
latency_multiplier=$SLURM_ARRAY_TASK_ID
speech_cache_size=$1 # 480 960 1440
max_llm_cache_size=$2 # 1000, 2000, 4000, 8000
...
simuleval \
    ...
    --output ${ckpt_dir}/simul-results-full-filterbadwords/speechcache${speech_cache_size}_llmcache${max_llm_cache_size}_seg${src_segment_size}_beam${beam}_ms${ms}_nrnl${no_repeat_ngram_lookback}_nrns${no_repeat_ngram_size} \
    ...
    --block-size 48 \
    --max-cache-size ${speech_cache_size} \
    --xpos 0 \
    \
