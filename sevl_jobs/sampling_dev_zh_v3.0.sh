...
simuleval \
    ...
    --target-lang Chinese \
    --min-start-sec ${ms} \
    --source /compute/babel-14-5/siqiouya/en-zh/dev_fa_traj_45.source.${SLURM_ARRAY_TASK_ID} \
    --target /compute/babel-14-5/siqiouya/en-zh/dev_fa_traj_45.target.${SLURM_ARRAY_TASK_ID} \
    --output ${ckpt_dir}/sampling_dev/cache${max_llm_cache_size}_seg${src_segment_size}_beam${beam}_ms${ms}/${SLURM_ARRAY_TASK_ID} \
