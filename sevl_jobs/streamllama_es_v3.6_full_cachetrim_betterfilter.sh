...
ckpt_dir=/compute/babel-5-23/siqiouya/runs/en-es/8B-traj-s2-v3.6/last.ckpt/
...
simuleval \
    ...
    --target-lang Spanish \
    --min-start-sec ${ms} \
    --source /compute/babel-14-5/siqiouya/en-es/tst-COMMON_full.source \
    --target /compute/babel-14-5/siqiouya/en-es/tst-COMMON_full.target \
    --output ${ckpt_dir}/simul-results-full-betterfilterbadwords/cache${max_llm_cache_size}_seg${src_segment_size}_beam${beam}_ms${ms}_nrnl${no_repeat_ngram_lookback}_nrns${no_repeat_ngram_size} \
