    ...
    def preprocess_gigaspeech_samples(self, json_path='data/gigaspeech_test_samples.json', target_sr=48000, max_workers=8):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for audio_path, start_time, end_time in audio_files:
                ...
                futures.append((future, audio_path))
            
            # 使用 tqdm 显示进度
            for future, audio_path in tqdm(futures, total=len(audio_files), desc="Processing Gigaspeech audio files"):
                try:
                    future.result()
...
if __name__ == "__main__":
    ...
    # 预处理所有 Gigaspeech 测试样本
    cache.preprocess_gigaspeech_samples(max_workers=8) 