...
class MimiCache:
    encoder_past_key_values: Optional[Union[Cache, List[torch.FloatTensor]]] = None
    pad_input_values: torch.Tensor = None

    def __init__(self, encoder_past_key_values=None, pad_input_values=None):
        self.encoder_past_key_values = encoder_past_key_values
        self.pad_input_values = pad_input_values
...
class MimiModel(MimiPreTrainedModel):
    ...
    def _encode_frame(
        ...
        seq_len = input_values.size(2)
        emb_len = seq_len // 1920 * 2

        if cache.pad_input_values is not None:
            input_values = torch.cat([cache.pad_input_values, input_values], dim=2)
        cache.pad_input_values = input_values[:, :, -1920:]

        embeddings = self.encoder(input_values)
        embeddings = embeddings[:, :, -emb_len:]

        encoder_outputs = self.encoder_transformer(
            embeddings.transpose(1, 2), past_key_values=cache.encoder_past_key_values, return_dict=return_dict
        )
        if return_dict:
            past_key_values = encoder_outputs.get("past_key_values")
        elif len(encoder_outputs) > 1:
            past_key_values = encoder_outputs[1]
        cache.encoder_past_key_values = past_key_values
    ...
    def encode(
        ...
        encoded_frames, cache = self._encode_frame(
            input_values,
            num_quantizers,
            padding_mask.bool(),
            cache=cache,
