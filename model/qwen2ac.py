...
from model.patches.patch_w2v2 import (
    get_attn_mask_inference,
    get_attn_mask_training
)
...
class Qwen2ACRoPELayerCache:
    k: torch.Tensor = None
    v: torch.Tensor = None

class Qwen2ACRoPECache:
    features: torch.Tensor = None
    embeds_len: int = 0
    n_steps: int = 0
    max_steps: int = 0
    layers: List[Qwen2ACRoPELayerCache] = None

    def __init__(self, features=None, embeds_len=0, n_steps=0, max_steps=0, layers=None):
...
class Qwen2AudioAttention(nn.Module):
    ...
    def forward(
        ...
        cache: Optional[Qwen2ACRoPELayerCache] = None,
        ...
        if cache.k is not None:
            key_states = torch.cat([cache.k, key_states], dim=2)
            value_states = torch.cat([cache.v, value_states], dim=2)
        cache.k, cache.v = key_states, value_states

        prefix_len = key_states.size(2) - query_states.size(2)
        q_pos_ids = torch.arange(prefix_len, prefix_len + tgt_len, device=query_states.device).repeat(bsz, 1)
        k_pos_ids = torch.arange(0, prefix_len + tgt_len, device=query_states.device).repeat(bsz, 1)

        q_cos, q_sin = self.rotary_emb(query_states, q_pos_ids)
        k_cos, k_sin = self.rotary_emb(key_states, k_pos_ids)
...
class Qwen2AudioEncoder(Qwen2AudioPreTrainedModel):
    ...
    def set_blocksize(self, multiplier: int):
        self.blocksize = self.config.blocksize * multiplier
    ...
    def forward(
        ...
        if cache.features is not None:
            input_features = torch.cat([cache.features, input_features], dim=2)
        cache.features = input_features[:, :, :-1]

        inputs_embeds = nn.functional.gelu(self.conv1(input_features))
        inputs_embeds = nn.functional.gelu(self.conv2(inputs_embeds))

        inputs_embeds = inputs_embeds[:, :, :-1]
        if cache.embeds_len > 0:
            new_embeds_len = inputs_embeds.size(-1)
            inputs_embeds = inputs_embeds[..., cache.embeds_len:]
            cache.embeds_len = new_embeds_len

            max_input_features_len = 100 # 1 second
            if cache.features.size(2) > max_input_features_len:
                cache.features = cache.features[:, :, -max_input_features_len:]
                cache.embeds_len = max_input_features_len // 2
        else:
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if to_drop:
                ...
                if cache.layers[idx].k is not None:
                    cache.layers[idx].k = cache.layers[idx].k[:, :, -cache.max_steps:]
                    cache.layers[idx].v = cache.layers[idx].v[:, :, -cache.max_steps:]
...
class Qwen2AudioForConditionalGeneration(Qwen2AudioPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if cache is None:
            cache = Qwen2ACRoPECache(
                max_steps=self.config.audio_config.sliding_window_size,
                layers=[Qwen2ACRoPELayerCache() for _ in range(self.config.audio_config.encoder_layers)]
            )
        self.audio_tower.set_blocksize(multiplier)
