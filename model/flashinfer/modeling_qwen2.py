...
from model.flashinfer.engine import LLMCache
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Self Attention
        hidden_states, _, pagetable = self.self_attn(
        ...
        return hidden_states, pagetable
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        for request in requests:
            ...
            qo_indptr.append(request['input_ids'].shape[0])

        paged_kv_indices = torch.tensor(paged_kv_indices, dtype=torch.int32, device=hidden_states.device)

        paged_kv_indptr = torch.tensor(paged_kv_indptr, dtype=torch.int32, device=hidden_states.device)
        paged_kv_indptr = paged_kv_indptr.cumsum(dim=0, dtype=torch.int32)

        paged_kv_last_page_len = torch.tensor(paged_kv_last_page_len, dtype=torch.int32, device=hidden_states.device)

        qo_indptr = torch.tensor(qo_indptr, dtype=torch.int32, device=hidden_states.device)
        qo_indptr = qo_indptr.cumsum(dim=0, dtype=torch.int32)

        if pagetable.wrapper_type == 'prefill':
