"""TTS generation using CosyVoice for wiki glossary terms.

Generates TTS audio for terms from a JSON glossary file using CosyVoice zero-shot inference
with a fixed reference audio.

python preprocess/rag_tts_wiki.py \
    --terms-path /data/group_data/li_lab/siqiouya/datasets/gigaspeech/wiki_glossary_nlp_ai_cs.json \
    --output-dir /data/group_data/li_lab/siqiouya/datasets/gigaspeech/wiki_glossary_tts_nlp_ai_cs \
    --overwrite
"""
...
def load_terms(terms_path: str) -> list[tuple[str, str]]:
    """Load terms from JSON glossary file.

    Reads a JSON file containing a list of {"term": ..., "tier": ...} entries,
    extracts the term field, and returns both original and cleaned versions.

    Args:
        terms_path: Path to the JSON glossary file

    Returns:
        List of (original_term, cleaned_term) tuples
    """
    ...
    for entry in data:
        original = entry["term"]
        # Strip parenthesized content for TTS, e.g. "BERT (language model)" -> "BERT"
        cleaned = re.sub(r'\s*\(.*?\)', '', original).strip()
        terms.append((original, cleaned))
    return terms
...
def main(args):
    ...
    for idx, (original_term, cleaned_term) in enumerate(tqdm(terms, desc="Generating TTS")):
        # Skip empty terms
        if not cleaned_term:
        ...
        if os.path.exists(tts_audio_path) and not args.overwrite:
            successful += 1
            term_to_path[original_term] = tts_audio_path
            continue
        ...
        try:
            ...
            for i, output in enumerate(cosyvoice.inference_zero_shot(
                cleaned_term,   # tts_text: cleaned term (no parentheses) for synthesis
                ...
                successful += 1
                # Add to mapping (use original term as key)
                term_to_path[original_term] = tts_audio_path
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Generate TTS for wiki glossary terms using CosyVoice")

    parser.add_argument(
        "--terms-path",
        type=str,
        default="/data/group_data/li_lab/siqiouya/datasets/gigaspeech/wiki_glossary_nlp_ai_cs.json",
        help="Path to the JSON glossary file",
    )

    parser.add_argument(
        "--output-dir",
        type=str,
        default="/data/group_data/li_lab/siqiouya/datasets/gigaspeech/wiki_glossary_tts",
