...
# Keep the global batch equal to prior runs: 8 * 1536 = 12288.
...
EVAL_TOP100_SAMPLES="${EVAL_TOP100_SAMPLES:-3}"
...
TCM_SWEEP_THRESHOLDS="${TCM_SWEEP_THRESHOLDS-0.85 0.80 0.75 0.70}"
...
EVAL_WIKI_GLOSSARY="${EVAL_WIKI_GLOSSARY-/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/data_pre/glossary_scale/wiki_glossary_nlp_ai_cs.json}"
...
BEST_METRIC_SECONDARY="${BEST_METRIC_SECONDARY-eval_acl6060/recall@10_gs10000}"
...
if [ "${SELECT_CLEAN_GPUS}" = "true" ]; then
...
echo "[TRAIN] Batch: ${BATCH_SIZE} (${NUM_GPUS} * ${PER_GPU_BATCH}) grad_cache_chunk=${GRAD_CACHE_CHUNK_SIZE} fixed_audio_seconds=${FIXED_AUDIO_SECONDS} eval_fixed_audio_seconds=${EVAL_FIXED_AUDIO_SECONDS}"
...
echo "[TRAIN] eval_sample_limit dev=${EVAL_SAMPLE_LIMIT} acl=${ACL_EVAL_SAMPLE_LIMIT} medicine=${MEDICINE_EVAL_SAMPLE_LIMIT} seed=${EVAL_SAMPLE_SEED}"
