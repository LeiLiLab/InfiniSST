...
# TTS-RAG per-paper SimulEval runner:
# - Uses pre-generated per-paper extracted glossaries.
# - Prepares per-paper dev.source/dev.target subsets.
# - For each paper, calls the ACL raw TTS SimulEval script with overrides:
#   - GLOSSARY_PATH_OVERRIDE
...
# Base TTS script
BASE_SCRIPT="${ROOT_DIR}/documents/code/simuleval/rank16/tts/paper_extract/acl_raw_bypass_slurm_rank32_iter_0000452_hf_zh.sh"
...
# Output base for this TTS paper-extract run
OUTPUT_BASE_DEFAULT="/mnt/gemini/data2/jiaxuanluo/infinisst_omni_vllm_rag_rank16_tts_intersection_extracted_glossary_by_paper"
...
# Pass-through overrides to base TTS script (optional)
...
RAG_EVAL_MODE_OVERRIDE="${RAG_EVAL_MODE_OVERRIDE:-intersection}"
RAG_INTERSECTION_STRATEGY_OVERRIDE="${RAG_INTERSECTION_STRATEGY_OVERRIDE:-pool_then_intersect}"
RAG_VOTING_K_OVERRIDE="${RAG_VOTING_K_OVERRIDE:-}"
RAG_TOP_K_OVERRIDE="${RAG_TOP_K_OVERRIDE:-}"
RAG_TTS_SIMILARITY_TOP_K_OVERRIDE="${RAG_TTS_SIMILARITY_TOP_K_OVERRIDE:-}"

# Compatibility with older sweep-style wrappers.
LATENCY_MULTIPLIERS_OVERRIDE="${LATENCY_MULTIPLIERS_OVERRIDE:-}"
RAG_K2_VALUES_OVERRIDE="${RAG_K2_VALUES_OVERRIDE:-}"
# Sweep defaults (RASST-style).
...
# Apply optional sweep overrides.
...
echo "[INFO] LATENCY_MULTIPLIERS=${LATENCY_MULTIPLIERS[*]} RAG_K2_VALUES=${RAG_K2_VALUES[*]}"
echo "[INFO] Overrides: resume='${RESUME_MODE}' model='${MODEL_NAME_OVERRIDE:-<default>}' rag_eval='${RAG_EVAL_MODE_OVERRIDE}' intersection_strategy='${RAG_INTERSECTION_STRATEGY_OVERRIDE}' rag_voting_k='${RAG_VOTING_K_OVERRIDE:-<default>}'"
...
PY
  ...
  echo "[INFO] GLOSSARY_PATH_OVERRIDE=${GLOSSARY_PATH}"
  ...
  for LATENCY_MULTIPLIER in "${LATENCY_MULTIPLIERS[@]}"; do
    for RAG_K2 in "${RAG_K2_VALUES[@]}"; do
      LOCAL_RAG_TOP_K="${RAG_K2}"
      if [[ -n "${RAG_TOP_K_OVERRIDE}" ]]; then
        LOCAL_RAG_TOP_K="${RAG_TOP_K_OVERRIDE}"
      fi
      ...
      RAG_TOP_K_OVERRIDE="${LOCAL_RAG_TOP_K}" \
      RAG_VOTING_K_OVERRIDE="${RAG_VOTING_K_OVERRIDE}" \
      RAG_TTS_SIMILARITY_TOP_K_OVERRIDE="${RAG_TTS_SIMILARITY_TOP_K_OVERRIDE}" \
