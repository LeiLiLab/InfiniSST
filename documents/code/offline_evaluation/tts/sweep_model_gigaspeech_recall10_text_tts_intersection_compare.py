...
UNKNOWN_EPOCH = -1
EPOCH_SORT_FALLBACK = 1_000_000_000
...
SKIP_MISSING_MODEL_PATH = True

# Sweep runtime placement.
# If True, epoch-sweep models are assigned to different GPUs in round-robin.
AUTO_ASSIGN_EPOCH_DEVICE = True
# Leave empty to auto-detect from CUDA_VISIBLE_DEVICES as cuda:0..cuda:N-1.
# Example: ["cuda:0", "cuda:1", "cuda:2", "cuda:3"]
EPOCH_SWEEP_DEVICES: List[str] = []
...
def _safe_int_from_any(value: Any, default: int = -1) -> int:
    try:
...
def _format_model_path_from_epoch(template: str, epoch: int) -> str:
    if EPOCH_PLACEHOLDER not in template:
        _err(
            f"NEW_TTS_MODEL_PATH_TEMPLATE must contain placeholder {EPOCH_PLACEHOLDER!r}, "
            f"got template={template!r}"
        )
    return template.replace(EPOCH_PLACEHOLDER, str(epoch))


def _expand_model_specs() -> List[Dict[str, Any]]:
    expanded: List[Dict[str, Any]] = []
    for spec in MODEL_SPECS:
        copied = dict(spec)
        copied.setdefault("epoch", -1)
        expanded.append(copied)

    for epoch in EPOCH_SWEEP_VALUES:
        expanded.append(
            {
                "model_name": f"{NEW_TTS_MODEL_NAME_PREFIX}_epoch_{epoch}",
                "model_path": _format_model_path_from_epoch(NEW_TTS_MODEL_PATH_TEMPLATE, int(epoch)),
...
def _ensure_runtime_device(requested_device: str) -> str:
    ...
    if ENABLE_CPU_FALLBACK_WHEN_NO_CUDA:
        _warn(
            "CUDA device is requested but no CUDA GPU is available in current process. "
            f"Fallback to {CPU_FALLBACK_DEVICE!r}. "
            "If you want CUDA, check CUDA_VISIBLE_DEVICES and runtime environment."
        )
        return CPU_FALLBACK_DEVICE
    _err(
        "CUDA device is requested but no CUDA GPU is available. "
        "Set ENABLE_CPU_FALLBACK_WHEN_NO_CUDA=True or fix CUDA runtime visibility."
    )
    return requested_device


def _resolve_sweep_devices(default_device: str) -> List[str]:
    if EPOCH_SWEEP_DEVICES:
        resolved: List[str] = []
        for raw_device in EPOCH_SWEEP_DEVICES:
            device = str(raw_device).strip()
            if not device:
                continue
            resolved.append(_resolve_effective_device(device))
        if resolved:
            return resolved
    ...
    if visible_raw:
        visible_list = [item.strip() for item in visible_raw.split(",") if item.strip()]
        if visible_list:
            return [f"cuda:{idx}" for idx, _ in enumerate(visible_list)]

    return [default_device]


def _select_device_for_model(
    spec: Dict[str, Any],
    fallback_device: str,
    sweep_devices: Sequence[str],
) -> str:
    if not AUTO_ASSIGN_EPOCH_DEVICE:
        return fallback_device

    epoch = _safe_int_from_any(spec.get("epoch", UNKNOWN_EPOCH), default=UNKNOWN_EPOCH)
    if epoch < 0:
        return fallback_device
    if not sweep_devices:
        return fallback_device
    return str(sweep_devices[epoch % len(sweep_devices)])
...
def _evaluate_single_model(
    ...
    use_cuda_amp = _is_cuda_device(str(effective_device))
...
def _log_delta(rows: Sequence[Dict[str, Any]]) -> None:
    ...
    baseline = None
    for row in rows:
        if str(row.get("model_name", "")) == "old_model_v1":
            baseline = row
            break
    if baseline is None:
        _warn("Delta skipped: baseline old_model_v1 not found.")
        return
    ...
    sweep_rows = [r for r in rows if int(r.get("enable_tts_eval", 0)) == 1]
    sweep_rows.sort(key=lambda r: _safe_int_from_any(r.get("epoch", UNKNOWN_EPOCH), default=EPOCH_SORT_FALLBACK))
    for row in sweep_rows:
        _log(f"Delta report: {row['model_name']} - {baseline['model_name']}")
        for key in metric_keys:
            try:
                delta = float(row[key]) - float(baseline[key])
                _log(f"delta_{key}={_format_float(delta)}")
            except Exception:
                _warn(f"Skip delta for key={key}")


def _to_float_or_default(value: Any, default: float = -1.0) -> float:
    try:
        return float(value)
    except Exception:
        return default
...
def _choose_best_sweep_row(rows: Sequence[Dict[str, Any]]) -> Dict[str, Any] | None:
    sweep_rows = [r for r in rows if _safe_int_from_any(r.get("epoch", UNKNOWN_EPOCH), default=UNKNOWN_EPOCH) >= 0]
    if not sweep_rows:
        return None

    # Primary: intersection recall; tie-breakers: tts recall, text recall, lower epoch.
    sweep_rows.sort(
        key=lambda r: (
            _to_float_or_default(r.get("intersection_recall_at_k"), default=-1.0),
            _to_float_or_default(r.get("tts_recall_at_k"), default=-1.0),
            _to_float_or_default(r.get("text_recall_at_k"), default=-1.0),
            -_safe_int_from_any(r.get("epoch", UNKNOWN_EPOCH), default=EPOCH_SORT_FALLBACK),
        ),
        reverse=True,
    )
    return sweep_rows[0]
...
def _write_plot(path: Path, rows: Sequence[Dict[str, Any]]) -> None:
    ...
    best_row = _choose_best_sweep_row(rows)
    if old_row is None or best_row is None:
        _warn("Plot skipped: expected rows for old_model_v1 and at least one swept epoch row.")
        return
    ...
    try:
        ...
        new_vals = [
            _parse_float_or_none(str(best_row.get("text_recall_at_k", ""))) or 0.0,
            _parse_float_or_none(str(best_row.get("text_precision_at_k", ""))) or 0.0,
        ]
        xs = [0, 1]
        bar_w = 0.36
        ...
        axes[0].bar(
            ...
            new_vals,
            width=bar_w,
            label=f"best_epoch_{_safe_int_from_any(best_row.get('epoch', -1), default=-1)}",
            color="tab:orange",
        )
        axes[0].set_xticks(xs)
        axes[0].set_xticklabels(labels)
        axes[0].set_ylim(0.0, 1.0)
        axes[0].set_title("Speech -> Text Top-10 (Baseline vs Best Epoch)")
        axes[0].grid(axis="y", alpha=0.3)
        axes[0].legend()

        # Panel 2: best epoch tts/intersection metrics.
        metric_keys = [
            "tts_recall_at_k",
            "tts_precision_at_k",
            "intersection_recall_at_k",
        ...
        metric_vals = [_parse_float_or_none(str(best_row.get(key, ""))) for key in metric_keys]
        ...
        axes[1].set_title("Best Epoch TTS / Intersection")
        ...
        keep_ratio = _parse_float_or_none(str(best_row.get("intersection_keep_ratio_vs_text", "")))
        if keep_ratio is not None:
            axes[1].text(
                ...
                (
                    "Best epoch="
                    f"{_safe_int_from_any(best_row.get('epoch', -1), default=-1)} | "
...
def main() -> int:
    ...
    model_specs = _expand_model_specs()
    sweep_devices = _resolve_sweep_devices(effective_device)
    _log(f"Sweep devices={sweep_devices} (AUTO_ASSIGN_EPOCH_DEVICE={AUTO_ASSIGN_EPOCH_DEVICE})")
    _log(
        f"Model sweep prepared: baseline={len(MODEL_SPECS)} + "
        f"epoch_sweeps={len(EPOCH_SWEEP_VALUES)} => total={len(model_specs)}"
    )
    for spec in model_specs:
        model_name = str(spec["model_name"])
        model_path = str(spec["model_path"])
        if not Path(model_path).exists():
            if SKIP_MISSING_MODEL_PATH:
                _warn(f"Skip model due to missing MODEL_PATH: model_name={model_name} model_path={model_path}")
                continue
            _err(f"MODEL_PATH not found: {model_path}")
        model_device = _select_device_for_model(
            spec=spec,
            fallback_device=effective_device,
            sweep_devices=sweep_devices,
        )
        _log(
            f"Dispatch model={model_name} epoch={_safe_int_from_any(spec.get('epoch', UNKNOWN_EPOCH), default=UNKNOWN_EPOCH)} "
        ...
        row = _evaluate_single_model(
            ...
            epoch=_safe_int_from_any(spec.get("epoch", -1), default=-1),
    ...
    if not rows:
        _err(
            "No model is evaluated successfully. "
            "All model paths may be missing or all runs failed before result collection."
        )
    ...
    if best_row is not None:
        _log(
            "Best epoch summary: "
            f"epoch={_safe_int_from_any(best_row.get('epoch', -1), default=-1)} "
            f"text_recall_at_k={best_row.get('text_recall_at_k', 'NA')} "
            f"tts_recall_at_k={best_row.get('tts_recall_at_k', 'NA')} "
            f"intersection_recall_at_k={best_row.get('intersection_recall_at_k', 'NA')}"
        )
