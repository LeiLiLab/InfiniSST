...
export const useTranslation = () => {
  ...
  // Initialize subtitle style first since it's needed in TranslationState
  ...
  const initializeSession = useCallback(async (
    ...
    try {
      ...
      // Since apiService doesn't have initializeSession, we're mocking it
  ...
  const resetTranslation = useCallback(async () => {
    ...
    try {
      // Since apiService doesn't have resetTranslation, we're just updating state
      // In a real implementation, you would call the actual API
  ...
  const updateLatency = useCallback(async (latencyMultiplier: number) => {
    ...
    try {
      // Since apiService doesn't have updateLatency, we're just updating state
      // In a real implementation, you would call the actual API
  ...
  return {
    ...
    error,
    volume,
    subtitleStyle: translationState.subtitleStyle,
    setSubtitleStyle: (newStyle: SubtitleStyle) => 
      setTranslationState(prev => ({ ...prev, subtitleStyle: newStyle })),
    initializeSession,
    startTranslation,
    togglePause,
    resetTranslation,
