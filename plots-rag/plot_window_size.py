...
# Data from table
window_sizes = [0.96, 1.92, 2.88, 3.84]
paper_recall = [89.23, 92.31, 92.31, 78.46]
tagged_recall = [71.92, 77.90, 74.18, 68.20]
...
# Add grid (only from left axis to avoid double grid)
...
for i, (x, y) in enumerate(zip(window_sizes, paper_recall)):
...
for i, (x, y) in enumerate(zip(window_sizes, tagged_recall)):
    if y == max(tagged_recall):
        ax2.plot(x, y, marker='s', markersize=14, color=color_tagged,
                 markerfacecolor='none', markeredgewidth=2.5, zorder=3)

# Save figure
plt.savefig('/home/siqiouya/code/infinisst-omni/plots-rag/figures/window_size.pdf',
            dpi=300, bbox_inches='tight')
print("Figure saved as 'window_size.pdf'")
