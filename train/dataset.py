...
class DataCollatorForTrajectoryInstructMultiLatencyMimiDataset:
    def __init__(self, 
            tokenizer, feature_extractor, source_lang, target_lang, 
            block_size=24, max_multiplier=1, prob_aug=0., **kwargs
        ):
    ...
    def __call__(self, samples: List[SpeechToTextDatasetItem]) -> Dict[str, torch.Tensor]:
        ...
        for i, x in enumerate(samples):
            ...
            for j, (text, _) in enumerate(x.trajectory):
                n_sp_token = self.speech_segment_size * multiplier
